...
Meteor.methods({
    ...
    'ss1Search'(searchText) {
        ...
        // NPM has been updated to work with promises :)  - https://www.npmjs.com/package/elasticsearch
                ...
                if (results.hits.total) {
                    ...
                    let requestPricesObv = new Observable.create(observer => {
                        client.query({
                            query: getRequestPrices,
                            forceFetch: true,
                            // fetchPolicy: 'cache-and-network',  doesn't work
                            // fetchPolicy: 'network-only',       doesn't work
                            variables: {
                                ownerId: ownerId,
                                status: 1,
                                serializedIds: serializedPriceIds
