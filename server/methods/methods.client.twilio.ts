...
Meteor.methods({
    ...
    /**
     * Verify User has verified phone number
     * 
     * @param code 
     */
    'verifyTwilioSMS' (code) {
        ...
        if (this.userId) {
            ...
            if (twInfo.code == parseInt(code)) {
                ...
                Meteor.call('user.update.cellVerified', true);          // async call
    ...
    'sendTwilioSMS' (cellPhone) {
        ...
        if (this.userId) {
            ...
            // If someone has this cellphone but has not verified it, grant it to this user??
            if ( (Meteor.user().username == undefined) || (Meteor.user().username != cellPhone) ) {
                ...
                if (userCheck == undefined) {
                    // 2 - update this user's username with this cellphone
                ...
                // Someone is already using this cellphone number
                    ...
                    // 1 - remove the username "cellphone from the other user
            ...
            if (twInfo == undefined) {
                ...
                console.log(currentTime + ' -- ' + twInfo.lastSMSDate + ' -- ' + expiredTime);
