...
Meteor.methods({
    ...
    /**
     * Insert Request price and price
     * 
     * @param p 
     * @param rp 
     * @param storeId 
    ...
    'ddp.requestprices.insert.price.x'(p: Price, rp: RequestPrice, storeId: string, info: infoModel) {
        ...
        if (this.connection == undefined) {
            ...
            if (this.connection.httpHeaders['user-agent'] == undefined) {
                return Meteor.call('requestprices.insert.price.x', p, rp, storeId, info);   
            }
            else {
                return {
                    status: false,
    ...
    'ddp.requestpricesQueue.reject'(rp1: RequestPriceProcess, info: infoModel) {
        ...
        if (this.connection == undefined) {
            ...
            // Verify ddp call was note made by a client
            if (this.connection.httpHeaders['user-agent'] == undefined) {
                return Meteor.call('requestpricesQueue.reject', rp1, info);       
            }
            else {
                return {
                    status: false,
    ...
    'ddp.requestprices.update'(rp: RequestPrice, info: infoModel) {
        ...
        if (this.connection == undefined) {
            ...
            // Verify ddp call was note made by a client
            if (this.connection.httpHeaders['user-agent'] == undefined) {
                return Meteor.call('requestprices.update', rp, info);        
            }
            else {
                return {
                    status: false,
    ...
    'ddp.requestprices.cancel'(rp: RequestPrice, info: infoModel) {
        ...
        if (this.connection == undefined) {
            ...
            // Verify ddp call was note made by a client
            if (this.connection.httpHeaders['user-agent'] == undefined) {
                return Meteor.call('requestprices.cancel', rp, info);   
            }
            else {
                return {
                    status: false,
    ...
    'ddp.requestprices.edit.price.item'(i: Item, p: Price, rp: RequestPrice, info: infoModel) {
        ...
        if (this.connection == undefined) {
            ...
            // Verify ddp call was note made by a client
            if (this.connection.httpHeaders['user-agent'] == undefined) {
                return Meteor.call('requestprices.edit.price.item', i, p, rp, info);  
            }
            else {
                return {
                    status: false,
