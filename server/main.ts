...
Meteor.startup(() => {
  ...
  Accounts.onCreateUser((options, user) => {
    ...
    // 1 - check if user account is non-Facebook
    if (user.services.facebook != undefined) {

      // 2 - check if Facebook email already exist in system - user signed up with it previously
      ...
      if (oldUser.length) {
        ...
        // 3 - Use the user ID we generated for Facebook account - copy over original Ranking, ledger Status
