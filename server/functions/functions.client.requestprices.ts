...
export function requestpricesInsertUpdateprice(rp: RequestPrice, storeId: string, owner: string, callFrom: string) {
    ...
    if (rp.note == 'skipQueue_insertES') {
        ...
        return startQueueRequestProcess(rpObject);                   // async calls (parent method is sync)
    }
}
...
export function requestpricesInsertNewprice(p: Price, rp: RequestPrice, storeId: string, owner: string, callFrom: string) {
    ...
    console.log('--- start  2244 -- is asyncy ???----- ' + storeId);
    ...
    // ff storeId and itemId are required when inserting new Price
    if (rp.note == 'skipQueue_insertES') {
...
export function requestPricesUpdate(rp: RequestPrice) {
    console.log('----requestprices.update-----> ' + rp.payRequestOrig);
    console.log(rp);
    ...
    RequestPrices.insert({
        ...
        .subscribe(
            x => {
                ...
                // 1) When Requestprice is inserted with a queue, it's status = 0 - do not update Ledger here
                // 2) Update balance when Requestprice is inserted with status equal to to 1 or 9
...
export function requestpricesInsertSkipQueue(rp: RequestPrice) {
    ...
    let peiObserv = new Observable.create(observer => {
        ...
        if (rp.note == 'skipQueue_insertES') {
            ...
            console.log('############ requestpricesInsertSkipQueue ##### 0000 note = ' + rp.note);
            ...
            if (resES.status) {
                ...
                console.error('ERROR: ############# elasticesearch - ERROR 1234 ############');
                console.error(resES);
                observer.next(resES);
                observer.complete();
            }
    ...
    combined$.subscribe(x => {
        ...
        console.error('############ requestpricesInsertSkipQueue complete ===========');
...
export function requestpricesQueueCancel(rp1: RequestPriceProcess) {
    ...
    console.log('========= paidTos ==========' + rp1.priceId);
    ...
    console.log('===> paidOut = ' + rp1.payRequest);

    // 3 - If DPP call is made, allow if Admin Key is legit, otherwise confirm owner of this Requestprice
    // let temp = verifyAdminKeyUser(rp1.userId, 'requestpricesQueueCancel', owner);      
    ...
    if (pqId == 'error') {
        ...
        if (resultCQ == 'ready') {
            ...
            if (rc.status) {
                ...
                reqPriceInfo.map(y => {
                    console.log("----------- yyy --------------" + y.expiresAt);
                    rp1.expiresAt = y.expiresAt;
                });
                console.log('===> expiresAt = ' + rp1.expiresAt);

                let results = requestpicesProcess(rp1);  
            ...
            // 7 - loop for a while until priceQueue opens up for desired priceId
            let counter = 0;
            let checkPriceQueueInterval = Meteor.setInterval(function () {
                ...
                if (resultCQ == 'ready') {
                    ...
                    if (rc.status) {
                        rp1.pqId = pqId;
                        let results = requestpicesProcess(rp1);                                         // ASYNC call - partial sync
                        QRPCfuture.return(results);
                    }
                    else {
                        // Delete price from queue
...
export function requestpricesQueueUpdate(rp1: RequestPriceProcess) {
    ...
    // 0 - Ensure user is not allowed to edit a Requestprices with PaidTos
    if (requestPrices.paidTos != undefined) {
        ...
        if (pqId == 'error') {
            ...
            console.log('******* pricequeues.insert: ' + pqId);
...
export function startQueueRequestProcess(rp: RequestPriceProcess) {
    ...
    if (!reqResults.status) {
        ...
        if (pqId == 'error') {
            ...
            console.log('******* pricequeuesInsert: ' + pqId);
            ...
            if (resultCQ == 'ready') {
                ...
                // 3a - check for race conditions - when Requestprice is READY to process
                ...
                let checkPriceQueueInterval = Meteor.setInterval(function () {
                    ...
                    if (resultCQ == 'ready') {
                        ...
                        if (rc.status) {
                            rp.pqId = pqId;
                            let results = requestpicesProcess(rp);                                  // ASYNC call - partial sync
                            QRPfuture.return(results);
                        }
                        else {
                            // Delete price from queue
...
export function requestpricesUpdates(sp1: SubmitPrice) {
    ...
    console.log("####### MESSAGE ==== global ==>>>" + sp1.priceId + ' - ' + sp1.owner);
    ...
    console.log('XXXXXXXX  requestPrices XXXXXXXX');
    console.log(requestPrices);
    ...
    let futures = _.map(requestPrices, function (requestPrice) {
        ...
        setTimeout(function () {
            ...
            Fiber(function () {
                ...
                let status = 2;
                let priceFactor = 3;            // full payout has been paid.
                let currentDate = new Date().getTime();
                ...
                console.log('********* ' + requestPrice._id + ' **PriceId: ' + sp1.priceId + ' **Payout: ' + payout + ' **PriceFactor: ' + priceFactor + ' **Expire: ' + requestPrice.expiresAt + ' **status: ' + status);
...
function requestpicesProcess(rp1: RequestPriceProcess) {
    ...
    console.log("###############################################################");
    console.log("####  note  = " + rp1.note  +  ' == ' + rp1.payRequest +  ' = ' + rp1.priceId + ' = ' + minExpiresAt + ' ####');
    console.log("###############################################################");
