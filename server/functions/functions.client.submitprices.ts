...
export function submitpricesInsertNewprice(p: Price, sp: SubmitPrice, storeId: string, owner: string) {
    ...
    // TODO - should I even pass this???
    sp.note = 'skipQueue_insertES';     
    sp.priceId = subPrice.id;
    sp.storeId = storeId;

    // since price doesn't exit, set status = 2
    sp.status = 2;                      
    sp.owner = owner;
...
export function submitpricesInsertUpdateprice(sp: SubmitPrice, storeId: string, owner: string) {
    ...
    if (sp.note == 'skipQueue_insertES') {
        ...
        // async call (parent method is sync)
...
export function submitpricesInsertSkipQueue(sp: SubmitPrice) {
    ...
    combined$.subscribe(x => {
        console.log('############ complete submitpricesInsertSkipQueue ===========');
...
export function startQueueSubmitProcess(sp1: SubmitPrice) {
    ...
    if (!results.status) {
        ...
        if (pqId == 'error') {
            ...
            console.log("########## submitpricesInsertUpdateprice 00-22 ########### --- " + pqId + "--- priceId = " +  sp1.priceId + '  storeId = ' + sp1.storeId + ' -- ' + sp1._id);

            if (resultCQ == 'ready') {
                // 3a - check for race conditions - when Submitprices is READY to process
                ...
                let checkPriceQueueInterval = Meteor.setInterval(function () {
                    counter++;
                    console.log(sp1.note + " pricequeuesCheck method called " + counter + " times... id=" + pqId + '  payoutRequest: ' + sp1.payout);
                    ...
                    if (resultCQ == 'ready') {
                        ...
                        console.warn("########## COUNTER EXCEEDED -- submitpricesInsertUpdateprice 00-77 ###########  " + counter);
...
export function submitpricesUpdate_pricesUpdate(sp1: SubmitPrice, res) {
    ...
    console.log("UPDATE PRICE.. NewPayout:" + newPayout + "  lastExpiresAt: " + lastExpiresAt + '  sp1.price: ' + sp1.price + ' soldOut: ' + sp1.soldOut);
    ...
    console.log('!!!!! You successfully submitted price: ' + sp1.price + '  sumPayout: ' + sumPayout + "  lastExpiresAt:" + lastExpiresAt);
    ...
    console.log(sp1._id + ' -- ' + sp1.note + ' -- ' + currentDate);
    console.log(rpids);
